...
-- Collision BOX <-> BOX using 1 MASKS SPD (!)
function collision_box_1mask_spd(box1, box2, mask1, vsp, hsp)
	return ((box1.x + mask1.x) - mask1.xoffset) + hsp < box2.x + box2.xoffset and
		   ((box1.x + mask1.x) + mask1.xoffset) + hsp > box2.x - box2.xoffset and
		   ((box1.y + mask1.y) - mask1.yoffset) + vsp < box2.y + box2.yoffset and
		   ((box1.y + mask1.y) + mask1.yoffset) + vsp > box2.y - box2.yoffset
end

--- (!)
function collision_box_mask_spd(box1, box2, mask1, mask2, hsp, vsp)

	local mask_2_offset = (mask2 ~= 0 and mask2 or box2)
	local mask_1_offset = (mask1 ~= 0 and mask1 or box1)

	return ((box1.x + (mask1 ~= 0 and mask1.x or 0) ) - mask_1_offset.xoffset) + hsp < (box2.x + (mask2 ~= 0 and mask2.x or 0) ) + mask_2_offset.xoffset and
		   ((box1.x + (mask1 ~= 0 and mask1.x or 0) ) + mask_1_offset.xoffset) + hsp > (box2.x + (mask2 ~= 0 and mask2.x or 0) ) - mask_2_offset.xoffset and
		   ((box1.y + (mask1 ~= 0 and mask1.y or 0) ) - mask_1_offset.yoffset) + vsp < (box2.y + (mask2 ~= 0 and mask2.y or 0) ) + mask_2_offset.yoffset and
		   ((box1.y + (mask1 ~= 0 and mask1.y or 0) ) + mask_1_offset.yoffset) + vsp > (box2.y + (mask2 ~= 0 and mask2.y or 0) ) - mask_2_offset.yoffset
end
